  ...
  for (const li of await page.locator('.guide-channels__channel').all()){
    ...
    if(await li.getByLabel('MTV3').isVisible()){
      ...
      for(const listItem of await li.locator('.schedule-card__header').all()){
        const time = await listItem.locator('.schedule-card__publication').getByText('22.00').isVisible();
        const title = await listItem.locator('.schedule-card__title').getByText('Kymmenen uutiset').isVisible();
        if(time && title){
          match = true;
        }
